...
async function handleTradeEntry(
  ...
  if (!payload.symbol || !payload.side || !payload.price || !payload.quantity) {
    return new Response(
      JSON.stringify({ success: false, status: 'invalid_request', reason: 'Missing required fields: symbol, side, price, quantity' }),
  ...
  if (error) {
    ...
    return new Response(
      JSON.stringify({ success: false, status: 'error', reason: error.message }),
  ...
  return new Response(
    JSON.stringify({ success: true, status: 'position_created', position_id: position.id }),
...
async function handleTradeExit(
  ...
  if (findError) {
    ...
    return new Response(
      JSON.stringify({ success: false, status: 'error', reason: findError.message }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
  ...
  if (updateError) {
    ...
    return new Response(
      JSON.stringify({ success: false, status: 'error', reason: updateError.message }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
...
async function handleUserActivity(
  ...
  return new Response(
    JSON.stringify({ success: true, status: 'activity_recorded' }),
    { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
  )
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Convert timestamp
    const rawTimestamp = ('data' in payload && payload.data?.timestamp) || ('timestamp' in payload ? payload.timestamp : undefined) || Date.now()
    const tsMillis = typeof rawTimestamp === 'number' && rawTimestamp < 10_000_000_000 
      ? rawTimestamp * 1000 
      : rawTimestamp
    ...
    if (isNaN(timestamp.getTime())) {
      return new Response(
        JSON.stringify({ success: false, status: 'invalid_request', reason: 'Invalid timestamp' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      )
    }

    const eventType = payload.type?.toUpperCase() || 'UNKNOWN'
    ...
    return new Response(
      JSON.stringify({ success: false, status: 'server_error', reason: String(error) }),
      { status: 500, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    )
  }
})
