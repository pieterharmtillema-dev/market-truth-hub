          ...
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent className="max-w-xs">
                <p>Observed {viewMode} performance. {onPeriodClick ? 'Click to filter.' : ''} Based on logged trades.</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
            ...
            <TooltipProvider key={period.key}>
              <Tooltip>
                <TooltipTrigger asChild>
                  <button
                    onClick={() => period.trades > 0 && onPeriodClick?.(period.date)}
                    className={`
                      rounded-lg p-2 md:p-3 text-center transition-all
                      ${period.trades > 0 ? 'hover:ring-2 hover:ring-primary/50' : ''}
                      ${getHeatmapColor(period.pnl, maxAbsPnl, period.trades > 0)}
                      ${period.trades > 0 && onPeriodClick ? 'cursor-pointer' : 'cursor-default'}
                    `}
                  >
