...
export function VerifiedMetricsCard({
  ...
  return (
    <Card className={cn(
      ...
      isVerified 
        ? "from-gain/10 to-gain/5 border-gain/30" 
        ...
        <div className={cn("grid gap-3", compact ? "grid-cols-2" : "grid-cols-2")}>
          ...
          <div className="bg-background/50 rounded-lg p-3 border border-border/50">
                  ...
                  <TooltipContent className="max-w-xs">
                    ...
                    {!hasEnoughTrades && (
                      <p className="text-xs text-warning mt-2">
                        Requires 30+ verified trades to display.
                      </p>
                    )}
                  </TooltipContent>
                </Tooltip>
            ...
            {!hasEnoughTrades && metrics && (
              <div className="text-xs text-muted-foreground mt-1">
                {metrics.total_verified_trades}/30 trades needed
              </div>
            )}
          </div>
        </div>
        ...
        {!isVerified && !compact && (
          <div className="bg-muted/30 rounded-lg p-3 border border-border/50">
            <p className="text-xs text-muted-foreground">
              ...
              Connect your exchange API to get verified metrics. 
              Unverified accounts cannot appear on the leaderboard.
            </p>
          </div>
        )}
      </CardContent>
